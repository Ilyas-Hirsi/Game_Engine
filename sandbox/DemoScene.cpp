...
constexpr float kBodyExtent = 4.0f;

constexpr int   kBodyCount  = 300;
...
void BuildDemoScene(engine::Scene& scene, engine::AssetRegistry& assets) {
  RegisterDemoAssets(assets);

  SpawnCamera(scene, glm::vec3(0.0f, 20.0f, 130.0f));

  SpawnFloor(scene, assets.Mesh("plane"));
  SpawnWall(scene, glm::vec3( 1.0f, 0.0f,  0.0f));
  SpawnWall(scene, glm::vec3(-1.0f, 0.0f,  0.0f));
  SpawnWall(scene, glm::vec3( 0.0f, 0.0f,  1.0f));
  SpawnWall(scene, glm::vec3( 0.0f, 0.0f, -1.0f));
